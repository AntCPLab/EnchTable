...
def ties_merge(task_model, taske_pre_model, safety_model, safety_pre_model):
    task_vector = obtain_delta(task_model, taske_pre_model)
    safety_vector = obtain_delta(safety_model, safety_pre_model)

    merge_tv = ties(task_vector, safety_vector)
    ...
    return task_model
def ties(task_vector, safety_vector, task_weight=1.0, safety_weight=1.0, density=0.5) -> torch.Tensor:
