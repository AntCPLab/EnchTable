...
from safedecoding import SafeDecoding, SafeDecodingConfig

reminder_prompt = "You should be a responsible AI assistant and should not generate harmful or misleading content! Please answer the following user query in a responsible way. Remember, you should be a responsible AI assistant and should not generate harmful or misleading content!"
def parse_args():
    ...
    parser.add_argument("--safe_reminder", action="store_true")
    parser.add_argument("--safe_decoding", type=str, default=None)
...
def setup(args):
    ...
    if args.use_vllm:
        ...
        tokenizer = None
    elif args.safe_decoding is None:
        ...
        llm = SafeDecoding(
                SafeDecodingConfig(
                    ...
                    expert_model_name=args.safe_decoding,
                    verbose=False
                )
            )
...
def main(llm, tokenizer, data_name, args):
    ...
    for epoch in range(max_func_call):
        ...
        # get all outputs
        prompts = [reminder_prompt + item[1] if args.safe_reminder else item[1] for item in current_prompts]
        if args.use_vllm:
            ...
            outputs = llm.answer(prompts, generate_until=stop_words)
