...
class SafeDecoding:
    def __init__(self, config: SafeDecodingConfig):
        ...
        try:
            self.base_model = AutoModelForCausalLM.from_pretrained(config.base_model_name, torch_dtype=torch.bfloat16, device_map=self.device)
            self.expert_model = AutoModelForCausalLM.from_pretrained(config.expert_model_name, torch_dtype=torch.bfloat16, device_map=self.device)
    ...
    def safedecoding_lora(self, messages, max_new_tokens=1024, do_sample=False, top_p=None, generate_until=None, target_word=None):
        ...
        try:
            ...
            input_ids = self.tokenizer(messages[-1]['content'], return_tensors="pt")['input_ids'].to(self.device)
        ...
        response = self.tokenizer.decode(generated_sequence)
        ...
        logging.info(f"Generated sequence: {response}")

        return response, output_length
    
    def answer(self, questions, generate_until=[], target_words=None):
        results = []
        if target_words is None:
            for q in tqdm(questions):
                ...
                response, length = self.safedecoding_lora(messages, max_new_tokens=512, do_sample=False, generate_until=generate_until)
                print(response)
                results.append(response)
        else:
            for i in tqdm(range(len(questions)//4)):
                q = questions[i*4]
                messages = [
                    {"role": "system", "content": "You are a helpful, respectful and honest assistant. Always answer as helpfully as possible, while being safe. Your answers should not include any harmful, unethical, racist, sexist, toxic, dangerous, or illegal content. Please ensure that your responses are socially unbiased and positive in nature.\n\nIf a question does not make any sense, or is not factually coherent, explain why instead of answering something not correct. If you don't know the answer to a question, please don't share false information."},
                    {"role": "user", "content": q},
                ]
                response, length = self.safedecoding_lora(messages, max_new_tokens=50, do_sample=False, generate_until=generate_until)
                print(response)
                res = None
                for word in response:
                    if word in ['A', 'B', 'C', 'D']:
                        gold = ['A', 'B', 'C', 'D'].index(word)
                        res = [(-10., False), (-10., False), (-10., False), (-10., False)]
                        res[gold] = (-0.01, True)
                        results.extend(res)
                        break
                if res is None:
                    res = [(-10., True), (-10., False), (-10., False), (-10., False)]
                results.extend(res)
        return results
...
if __name__ == "__main__":
    ...
    safe_decoder = SafeDecoding(
        SafeDecodingConfig(
            base_model_name="/root/jinlin/LLaMA-Factory/saves/llama3-8b-beavertail_harmful/full/sft",
