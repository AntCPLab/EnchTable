...
from utils.safedecoding import SafeDecoding, SafeDecodingConfig
...
class SaladDataset:
    ...
    def get_questions(self):
        return [entry['prompt'] for entry in self.data]
...
def generate_response_batch(questions, safe_decoder, max_new_tokens=256, do_sample=False, top_p=None):
    ...
    responses, lengths = safe_decoder.safedecoding_lora(
        messages,
    ...
    #     ]
    #     response, length = safe_decoder.safedecoding_lora(
    #         messages,
    #         max_new_tokens=max_new_tokens,
    #         do_sample=do_sample,
    #         top_p=top_p
...
def process_dataset(dataset, safe_decoder, batch_size, generation_method='sample', **kwargs):
    results = []
        ...
        for batch_questions in t:
            try:
                ...
                batch_responses, batch_lengths = generate_response_batch(
                    batch_questions, safe_decoder, do_sample=do_sample, top_p=kwargs.get('top_p')
                )
...
def main():
    ...
    results, stats = process_dataset(dataset, safe_decoder, args.batch_size, args.generation_method, 
                                    top_p=args.top_p)
    
    simplified_model_id = simplify_model_id(args.model_id)
    file_name = os.path.basename(args.input_file)
    output_path = f"./test_salad/llama3/generate/safedecoding/{file_name}_{simplified_model_id}_responses.json"
