...
def evaluate_response_batch(batch, judge_llm, judge_tokenizer):
    ...
    for entry, output in zip(batch, outputs):
        ...
        if result and ', score ' in result:
            ...
            logging.error(f"Unexpected result format for question: {entry['prompt']}. Result: {result}")
        ...
        result_entry = {
            ...
            'subtopic': entry.get('subtopic', None)
        }
...
def process_dataset(dataset, judge_llm, judge_tokenizer):
    ...
    for batch in tqdm(dataset.get_batches(), desc="Evaluating responses"):
        try:
            ...
            for entry in batch:
                logging.error(f"Exception occurred while evaluating question: {entry['prompt']}. Error: {e}")
                result_entry = {
                    'qid': entry.get('qid', None),
                    'source': entry.get('source', None),
                    'question': entry['prompt'],
